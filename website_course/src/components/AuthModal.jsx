...
function AuthModal({ isOpen, onClose, defaultTab = 'login', onSwitchTab }) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-label="Masuk atau Daftar">
      <div className={`absolute inset-0 bg-black/70 transition-opacity duration-200 ${isClosing ? 'opacity-0' : 'opacity-100'}`} onClick={handleClose} />
        ...
        <div className="flex mb-6 bg-[rgba(30,27,75,0.5)] rounded-full p-1 border border-white/5">
          <button
            ...
            className={`flex-1 py-2.5 rounded-full text-sm font-semibold transition-all duration-200 ${
              tab === 'login' ? 'bg-button-gradient text-white shadow-glass' : 'text-gray-400 hover:text-white'
            }`}
          >
            Masuk
          </button>
          <button
            ...
            className={`flex-1 py-2.5 rounded-full text-sm font-semibold transition-all duration-200 ${
              tab === 'register' ? 'bg-button-gradient text-white shadow-glass' : 'text-gray-400 hover:text-white'
            }`}
          >
            Daftar
          </button>
        ...
        {tab === 'login' ? (
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full btn-premium bg-button-gradient text-white py-3 rounded-lg font-semibold text-sm shadow-glass disabled:opacity-60 disabled:cursor-not-allowed transition-all"
            >
